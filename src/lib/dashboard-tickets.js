  ...
  select?.addEventListener('change', async () => {
    ...
    try {
      ...
      const { data: tickets, error: tickErr } = await safeQuery(
        supabase.from('tickets').select('id, ticket_tier_id, order_id, user_id, status, qr_hash, created_at, scanned_at, attendee_name, attendee_email').in('ticket_tier_id', tiers.map(t => t.id)).order('created_at', { ascending: false })
      );
      if (getSwitchId() !== mySwitch) return;  // stale — user switched tabs
