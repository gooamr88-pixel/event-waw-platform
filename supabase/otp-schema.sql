-- ═══════════════════════════════════
-- EVENT WAW — OTP System (No Edge Functions)
...
-- Users can read their own OTPs (needed to get the code for sending)
...
-- Users can update their own OTPs (for attempt tracking)
...
-- ══════════════════════════════════
-- RPC: Generate OTP
-- Returns the 6-digit code for the client to send via Brevo
-- ══════════════════════════════════
...
BEGIN
  ...
  -- Store new OTP
  INSERT INTO login_otps (user_id, email, code_hash, otp_code, expires_at)
  VALUES (
    v_user_id,
    v_email,
